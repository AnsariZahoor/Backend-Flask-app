...
app.config['UPLOAD_FLODER'] = params['upload_location']
app.config.update(
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title = box_title, slug = slug, content = content, date=date, img_file = img_file)
                db.session.add(post)
                db.session.commit()
            else:
...
def uploader():
    ...
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            f.save(os.path.join(
                app.config['UPLOAD_FLODER'], secure_filename(f.filename)))
